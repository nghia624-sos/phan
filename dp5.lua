-- TT:dongphandzs1 - Fram NPC gần nhất + Tăng vùng sát thương + Hitbox đỏ
        ...
        while isRunning do
            ...
            if target and target:FindFirstChild("HumanoidRootPart") then
                ...
                for angle = 0, 360, 15 do
                    ...
                    TweenService:Create(hrp, TweenInfo.new(1 / speed), {CFrame = cf}):Play()
                    wait(1 / speed)
                end
            else
                wait(1)
...
createButton("Bật/Tắt Hitbox đỏ", 95, function()
    showHitbox = not showHitbox
    updateHitbox()
end)

createSlider("Khoảng cách", 5, 50, radius, 130, function(val)
    radius = val
end)

createSlider("Tốc độ", 1, 15, speed, 180, function(val)
    speed = val
end)

createSlider("Kích thước hitbox", 1, 30, hitboxSize.X, 230, function(val)
    hitboxSize = Vector3.new(val, val, val)
    updateHitbox()
end)

-- Hiển thị máu mục tiêu + quay mặt
local hpLabel = Instance.new("TextLabel", frame)
hpLabel.Size = UDim2.new(0.9, 0, 0, 20)
hpLabel.Position = UDim2.new(0.05, 0, 1, -25)
