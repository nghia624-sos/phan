...
-- Khi tới gần mục tiêu mới bắt đầu chạy vòng tròn + Tự né vật cản + Bật/Tắt menu
...
-- Né vật cản đơn giản
local function isPathClear(pos)
    local ray = Ray.new(hrp.Position, (pos - hrp.Position).Unit * (pos - hrp.Position).Magnitude)
    local part = workspace:FindPartOnRay(ray, character)
    return not part or part:IsDescendantOf(character)
end
...
-- Di chuyển tự nhiên bằng MoveTo + kiểm tra né vật cản
