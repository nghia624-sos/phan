-- GUI TT:dongphandzs1 với tự động chạy đến + chạy vòng CityNPC --
...
-- Tìm mục tiêu chứa 'CityNPC' (không phân biệt chữ)
	...
	while true do
		...
		if runFram and character and hrp then
			...
			if target then
				...
				if hum then
					if dist > r + 2 then
						-- Đang xa -> chạy bộ tự nhiên đến mục tiêu
						hum:MoveTo(target.HumanoidRootPart.Position + Vector3.new(0, 0, -2))
					else
						-- Đã gần -> bắt đầu chạy vòng tròn quanh mục tiêu
