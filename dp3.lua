...
-- Tạo nút bật/tắt hitbox
createButton("Bật/Tắt hiển thị vùng sát thương", 190, function()
    showHitbox = not showHitbox
    if not showHitbox and hitboxPart then
        hitboxPart:Destroy()
        hitboxPart = nil
    end
end)

-- Input kích thước vùng sát thương
createSlider("X Hitbox", 1, 50, hitboxSize.X, 220, function(val)
    hitboxSize = Vector3.new(val, hitboxSize.Y, hitboxSize.Z)
end)
createSlider("Y Hitbox", 1, 50, hitboxSize.Y, 270, function(val)
    hitboxSize = Vector3.new(hitboxSize.X, val, hitboxSize.Z)
end)
createSlider("Z Hitbox", 1, 50, hitboxSize.Z, 320, function(val)
    hitboxSize = Vector3.new(hitboxSize.X, hitboxSize.Y, val)
end)

-- Theo dõi vũ khí và gắn box
RunService.RenderStepped:Connect(function()
