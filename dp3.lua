...
-- Tự động chạy đến mục tiêu, khi đủ gần sẽ tự chạy vòng tròn quanh mục tiêu và tự động đánh
...
local function createInput(text, default, onChanged)
...
createInput("Bán kính vòng (radius)", radius, function(val)
	radius = val
end)

createInput("Tốc độ vòng (speed)", speed, function(val)
	speed = val
end)
...
-- Chạy theo mục tiêu và chuyển sang vòng tròn khi đủ gần
	...
	if running then
		...
		if currentTarget and currentTarget:FindFirstChild("HumanoidRootPart") then
			...
			if dist > radius + 2 then
				...
				local targetPos = targetHRP.Position + offset + Vector3.new(0, 1, 0)
