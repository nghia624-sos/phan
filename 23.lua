...
-- Di chuyển theo mục tiêu đang di chuyển
local function followTarget(target)
    while running and target and target:FindFirstChild("HumanoidRootPart") and target:FindFirstChild("Humanoid") and target.Humanoid.Health > 0 do
        ...
        if distance > radius + 2 then
            ...
            return true -- Gần đủ để bắt đầu chạy vòng
        end
        task.wait(0.1)
    end
    return false
    ...
    while true do
        ...
        if running then
            ...
            if currentTarget and currentTarget:FindFirstChild("HumanoidRootPart") then
                local followed = followTarget(currentTarget)
                if followed and running and currentTarget:FindFirstChild("Humanoid") then
