	...
	while running do
		...
		if currentTarget and currentTarget:FindFirstChild("HumanoidRootPart") then
			repeat
				...
				wait(0.2)
			until not running or (hrp.Position - currentTarget.HumanoidRootPart.Position).Magnitude <= (radius + 2)
			...
			if running and currentTarget then
				...
				repeat
					wait(0.2)
				until not currentTarget or currentTarget:FindFirstChild("Humanoid") == nil or currentTarget.Humanoid.Health <= 0 or not running
			end
		end
		wait(0.5)
	end
