...
RunService.Heartbeat:Connect(function(dt)
	if running and currentTarget and currentTarget:FindFirstChild("HumanoidRootPart") then
		...
		-- Tính vị trí trên quỹ đạo
		local angle = tick() * speed
		local offset = Vector3.new(math.cos(angle) * radius, 0, math.sin(angle) * radius)
		local goalPos = currentTarget.HumanoidRootPart.Position + offset

		-- Di chuyển mượt từng bước tới goalPos
		local direction = goalPos - hrp.Position
		local distance = direction.Magnitude
		local step = direction.Unit * math.min(distance, 0.5)

		if distance > 0.5 then
			hrp.CFrame = CFrame.new(hrp.Position + step, currentTarget.HumanoidRootPart.Position)
		else
