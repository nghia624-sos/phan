...
RunService.Heartbeat:Connect(function()
	if running and currentTarget and currentTarget:FindFirstChild("HumanoidRootPart") then
		if currentTarget.Humanoid.Health <= 0 then
			running = false
			hpLabel.Text = "Mục tiêu đã bị tiêu diệt!"
			toggle.Text = "BẬT: Đánh Boss"
			return
		end
		...
		local goalPos = Vector3.new(targetPos.X + offset.X, targetPos.Y, targetPos.Z + offset.Z)
		TweenService:Create(hrp, TweenInfo.new(0.1), {Position = goalPos}):Play()
		hrppos = hrp.Position
		local dir = (targetPos - hrppos).Unit
