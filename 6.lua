...
radiusBox.PlaceholderText = "Bán kính X (mặc định 10)"
radiusBox.Text = ""
...
local radiusZBox = Instance.new("TextBox", frame)
radiusZBox.Position = UDim2.new(0, 140, 0, 65)
radiusZBox.Size = UDim2.new(0, 110, 0, 25)
radiusZBox.PlaceholderText = "Bán kính Z (elip)"
radiusZBox.Text = ""
radiusZBox.Font = Enum.Font.SourceSans
radiusZBox.TextSize = 14
radiusZBox.BackgroundColor3 = Color3.fromRGB(70,70,70)
...
function moveInElip(target, radiusX, radiusZ, speed)
	local angle = 0
	local root = target:FindFirstChild("HumanoidRootPart")
	while running and target and target:FindFirstChild("HumanoidRootPart") and target.Humanoid.Health > 0 do
		local pos = root.Position + Vector3.new(math.cos(angle) * radiusX, 0, math.sin(angle) * radiusZ)
		humanoid:MoveTo(pos)
			...
			while running do
				...
				if target then
					...
					repeat
						...
						task.wait(0.2)
						success = (hrp.Position - target.HumanoidRootPart.Position).Magnitude < 12
					until not running or success

					local rX = tonumber(radiusBox.Text) or 10
					local rZ = tonumber(radiusZBox.Text) or 5
