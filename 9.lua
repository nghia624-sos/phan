...
FramTab:CreateToggle({
    ...
    orbitConnection = RunService.RenderStepped:Connect(function(dt)
        if not framEnabled or not target or target.Humanoid.Health <= 0 then
            orbiting = false
        ...
        HRP.CFrame = CFrame.new(HRP.Position, target.HumanoidRootPart.Position) -- quay mặt
        Humanoid:MoveTo(targetPos)

        -- Auto đánh nếu đang cầm vũ khí
    ...
    if target then
        ...
        if tHRP then
            ...
            -- Di chuyển tới mục tiêu cho đến khi gần
            ...
            -- Nếu vẫn sống thì bắt đầu quay vòng
            if framEnabled and target.Humanoid.Health > 0 then
                startOrbit(target)

                -- Chờ cho đến khi mục tiêu chết
    ...
    wait(0.5) -- Chờ trước khi tìm mục tiêu mới
end
})
