...
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local HRP = LocalPlayer.Character and LocalPlayer.Character:WaitForChild("HumanoidRootPart") or nil

-- 1. Nhìn mục tiêu xuyên tường (ESP cơ bản)
local function CreateESP(player)
    if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and not player.Character:FindFirstChild("ESP") then
...
-- 2. Hitbox + Line đến tất cả người chơi (PvP style)
GunTab:CreateToggle({
    Name = "🧊Hitbox + Line",
    Default = false,
    Callback = function(Value)
        if Value then
            for _, player in pairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not part:FindFirstChild("GunHitbox") then
                    ...
                    if not part:FindFirstChild("GunLine") then
            ...
            for _, p in pairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character then
                    for _, v in pairs(p.Character:GetDescendants()) do
                        if v:IsA("BoxHandleAdornment") and v.Name == "GunHitbox" then
                            v:Destroy()
                        end
                        if v:IsA("Beam") and v.Name == "GunLine" then
                            v:Destroy()
                        end
                    end
                end
