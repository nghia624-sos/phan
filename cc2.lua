...
------------ TAB 3: Troll Player ------------
local TrollTab = Window:CreateTab("Troll Player", 4483362458) -- icon ID bạn đổi được
        ...
        if state then
            ...
            for _, player in pairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    flyPart.Velocity = Vector3.new(0, 100, 0) -- Bay lên
                    flyPart.MaxForce = Vector3.new(1e6, 1e6, 1e6)
                    flyPart.Parent = player.Character.HumanoidRootPart
                    flyPart.Name = "TrollFlyPart"
                end
            end
        else
            -- Khi tắt
            for _, player in pairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                    local flyPart = player.Character.HumanoidRootPart:FindFirstChild("TrollFlyPart")
                    if flyPart then
                        flyPart:Destroy()
                    end
                end
