...
local function runAround(target)
    ...
    while running and target and target:FindFirstChild("HumanoidRootPart") and target:FindFirstChild("Humanoid") and target.Humanoid.Health > 0 do
        ...
        chr:SetPrimaryPartCFrame(CFrame.new(hrp.Position, target.HumanoidRootPart.Position))
        task.wait(0.03)
        if (hrp.Position - target.HumanoidRootPart.Position).Magnitude > radius + 5 then
            break
        end
    end
end

-- Theo mục tiêu liên tục
