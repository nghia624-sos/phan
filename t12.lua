...
toggle.MouseButton1Click:Connect(function()
	...
	loopConn = RunService.Heartbeat:Connect(function(dt)
		if not running or not currentTarget or not currentTarget.Parent then return end
		...
		if not h or not r or h.Health <= 0 then
			hpLabel.Text = "Máu mục tiêu: Không tìm thấy hoặc chết"
			currentTarget = findNPC2()
			clearPath()
			return
		end
		...
		if dist > radius + 3 then
			-- Tiếp cận bằng Pathfinding
			if tick() >= recomputeAt or not currentPath or wpIndex >= #waypoints then
			...
			local dir = (goalPos - hrp.Position)
			if dir.Magnitude > 0.1 then
				local moveStep = dir.Unit * math.min(dir.Magnitude, 6 * dt)
				hrp.CFrame = CFrame.new(hrp.Position + moveStep, Vector3.new(r.Position.X, hrp.Position.Y, r.Position.Z))
			end
