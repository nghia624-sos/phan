-- Fram NPC Menu Script - Di chuyển tự nhiên (không teleport)
	...
	while task.wait(0.5) do
		if running then
			...
			if target then
				...
				if distance > 15 then -- Chỉ tìm đường nếu còn xa
...
-- Chạy vòng tròn quanh mục tiêu tự nhiên
spawn(function()
	local angle = 0
	while task.wait(0.05) do
		if autoCircle then
			...
			if target and target:FindFirstChild("HumanoidRootPart") then
				...
				goal = Vector3.new(goal.X, hrp.Position.Y, goal.Z)

				humanoid:MoveTo(goal)
				char:SetPrimaryPartCFrame(CFrame.new(hrp.Position, target.HumanoidRootPart.Position))
