...
noclipBtn.Position = UDim2.new(0, 0, 0, 200) -- đặt phía trên nameText để không đè tab khác
...
-- Thêm nút GUI cho Heal-on-Hit
local healBtn = Instance.new("TextButton", frame)
healBtn.Size = UDim2.new(1, 0, 0, 30)
healBtn.Position = UDim2.new(0, 0, 0, 170) -- đặt dưới autoBtn hoặc chỉnh lại layout nếu cần
healBtn.Text = "BẬT: Đánh hồi 20 máu"
...
healBtn.MouseButton1Click:Connect(function()
    ...
    healBtn.Text = (healEnabled and "TẮT" or "BẬT") .. ": Đánh hồi 20 máu"
end)
...
local function attack()
    local tool = lp.Character and lp.Character:FindFirstChildOfClass("Tool")
    if tool then pcall(function() tool:Activate() end) end
    if healEnabled and hum.Health > 0 then
        hum.Health = math.min(hum.MaxHealth, hum.Health + 20) -- hồi 20 máu, không vượt quá MaxHealth
    end
end