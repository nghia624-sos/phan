...
-- Hàm tiếp cận mục tiêu bằng Tween
local function moveToTarget(target)
	...
	while running and target and target:FindFirstChild("HumanoidRootPart") and target.Humanoid.Health > 0 do
		...
		local step = math.min(distance - radius, 5)
		local tween = TweenService:Create(hrp, TweenInfo.new(0.2), {CFrame = CFrame.new(hrp.Position, target.HumanoidRootPart.Position) * CFrame.new(0,0,-step)})
		tween:Play()
...
local function moveToTarget(target)
	...
	while running and target and target:FindFirstChild("HumanoidRootPart") and target.Humanoid.Health > 0 do
		...
		-- Tính hướng di chuyển và bước nhỏ
		local direction = (target.HumanoidRootPart.Position - hrp.Position).Unit
		local stepDistance = math.min(distance - radius, 2) -- 2 studs/bước để tránh kick
		local newPos = hrp.Position + direction * stepDistance

		-- Dùng tween ngắn để mượt (0.05s)
		local tween = TweenService:Create(hrp, TweenInfo.new(0.05, Enum.EasingStyle.Linear), {
			CFrame = CFrame.new(newPos, target.HumanoidRootPart.Position)
		})
		tween:Play()
