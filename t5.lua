...
-- Tab giảm đồ họa
local gfxBtn = Instance.new("TextButton", frame)
gfxBtn.Size = UDim2.new(1, 0, 0, 30)
gfxBtn.Position = UDim2.new(0, 0, 0, 270)
gfxBtn.Text = "TẮT: Giảm Đồ Họa"
gfxBtn.BackgroundColor3 = Color3.fromRGB(50,50,50)
gfxBtn.TextColor3 = Color3.new(1,1,1)

local gfxReduced = false
gfxBtn.MouseButton1Click:Connect(function()
    gfxReduced = not gfxReduced
    gfxBtn.Text = (gfxReduced and "BẬT" or "TẮT") .. ": Giảm Đồ Họa"

    for _,v in pairs(workspace:GetDescendants()) do
        if v:IsA("Accessory") or v:IsA("Decal") then
            v.Transparency = gfxReduced and 1 or 0
        end
        if v:IsA("BasePart") then
            v.Material = gfxReduced and Enum.Material.SmoothPlastic or Enum.Material.Plastic
        end
    end

    Lighting.GlobalShadows = not gfxReduced
    Lighting.FogEnd = gfxReduced and 100 or 100000
end)
