...
task.spawn(function()
    ...
    repeat task.wait() until gui and gui:IsA("ScreenGui")
    ...
    local function update(input)
        ...
        gui:FindFirstChildWhichIsA("Frame").Position = UDim2.new(
            startPos.X.Scale, startPos.X.Offset + delta.X,
            startPos.Y.Scale, startPos.Y.Offset + delta.Y
        )
    end

    gui:FindFirstChildWhichIsA("Frame").InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragToggle = true
            dragStart = input.Position
            startPos = gui:FindFirstChildWhichIsA("Frame").Position
    ...
    gui:FindFirstChildWhichIsA("Frame").InputChanged:Connect(function(input)
