...
local startTime = 0 -- Thời điểm bắt đầu chạy vòng
...
RunService.Heartbeat:Connect(function(dt)
	if running and currentTarget and currentTarget:FindFirstChild("HumanoidRootPart") then
		...
		local moveStep = direction * math.min((targetPos - hrp.Position).Magnitude, 0.25)
...
-- Nút Bật/Tắt chạy vòng
toggle.MouseButton1Click:Connect(function()
	running = not running
	if running then
		currentTarget = getTarget()
		startTime = tick() -- Ghi thời gian bắt đầu
	end
