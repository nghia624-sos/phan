...
-- Auto Aim riêng (nếu không dùng "Chạy Vòng")
task.spawn(function()
	while true do wait()
		if aiming and not running then
...
-- Auto Vòng + hướng về mục tiêu (KHÔNG TELEPORT)
task.spawn(function()
	while true do wait()
		if running then
			...
			if target and target:FindFirstChild("HumanoidRootPart") then
				...
				if distanceToTarget > dist + 2 then
					-- Chạy tự nhiên lại gần mục tiêu
					humanoid:MoveTo(tpos + Vector3.new(0, 0, -dist))
				else
					-- Gần rồi → quay vòng
