...
sliderKnob.Position = UDim2.new(0.25, -5, 0, -5)
...
game:GetService("UserInputService").InputChanged:Connect(function(input)
	if dragging and (input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement) then
		...
		keepDistance = math.floor((x / slider.AbsoluteSize.X) * 40) + 5 -- Từ 5 đến 45
		distanceLabel.Text = "Khoảng cách: " .. keepDistance
	end
end)

-- Tìm mục tiêu gần nhất (Player)
...
game:GetService("RunService").Heartbeat:Connect(function()
	...
	if target and target:FindFirstChild("HumanoidRootPart") then
		...
		-- Logic giữ khoảng cách
		if dist < keepDistance - 1 then
			-- LÙI lại nhẹ bằng CFrame + Move
			local back = hrp.Position - direction * 0.7
			hrp.CFrame = CFrame.new(back, Vector3.new(tHRP.Position.X, back.Y, tHRP.Position.Z))
			hum:Move(-direction, false)
		elseif dist > keepDistance + 1 then
			-- Tiến tới
			hum:Move(direction, false)
		else
			-- Trong vùng an toàn → đứng đánh
