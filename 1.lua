...
function circleTarget(npc)
    coroutine.wrap(function()
        while autoFram and npc and npc:FindFirstChild("HumanoidRootPart") and npc.Humanoid.Health > 0 do
            ...
            local tween = TweenService:Create(hrp, TweenInfo.new(0.2, Enum.EasingStyle.Linear), {CFrame = CFrame.new(goalPos, npc.HumanoidRootPart.Position)})
            tween:Play()
            wait(0.2)
        end
    end)()
    ...
    while true do wait(0.5)
        if autoFram then
            ...
            if target then
                ...
                autoAttack(target)
                -- Wait target die
                repeat wait() until target.Humanoid.Health <= 0 or not autoFram
            end
        end
    end
end)
