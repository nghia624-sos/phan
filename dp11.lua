...
-- Tìm NPC gần nhất có tên chứa "citynpc" hoặc "npcity"
...
local function pathfindTo(targetPos)
    ...
    if path.Status == Enum.PathStatus.Complete then
        ...
        warn("Không thể tìm đường đến mục tiêu.")
    end
end

-- Di chuyển tới gần mục tiêu (<=10m), né rào cản
local function walkToTarget(npc)
    local targetPos = npc.HumanoidRootPart.Position
    local distance = (targetPos - hrp.Position).Magnitude

    if distance > 10 then
        pathfindTo(targetPos + Vector3.new(3, 0, 3)) -- Đi tới gần
...
-- Chạy vòng quanh NPC + auto aim + auto đánh
...
local function startFram()
    ...
    while running do
        ...
        if npc and npc:FindFirstChild("Humanoid") and npc.Humanoid.Health > 0 then
            walkToTarget(npc)    -- Di chuyển né vật cản
            orbitTarget(npc)     -- Chạy vòng + đánh
            repeat task.wait(0.2)
